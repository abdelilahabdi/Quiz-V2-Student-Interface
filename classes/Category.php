...
class Category {
    ...
    public function getCategoriesWithActiveQuizzes() {
        $sql = "SELECT c.*, 
                       COUNT(q.id) as active_quiz_count
                FROM categories c
                INNER JOIN quiz q ON q.categorie_id = c.id 
                WHERE q.is_active = 1
                GROUP BY c.id
    ...
    /**
     * Récupère une catégorie avec le nombre de quiz actifs
     */
    public function getCategoryWithActiveQuizCount($categoryId) {
        $sql = "SELECT c.*, 
                       COUNT(q.id) as active_quiz_count
                FROM categories c
                LEFT JOIN quiz q ON q.categorie_id = c.id AND q.is_active = 1
                WHERE c.id = ?
                GROUP BY c.id";
